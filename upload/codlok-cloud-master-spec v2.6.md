...
**Spec Version: 2.6** — Last updated: Verify (§20) stress-tested and frozen. State machine revised after live research against real Stripe Identity and Persona status models — added Adapter Absorption Rule since neither provider's real lifecycle was one-directional. verificationType changed from opaque string to canonical enum. Verification Fact Immutability Rule added (mirrors Pay's Financial Facts Rule). Ready for agent validation and build.
...
| Verify | v1.0 | 🟢 Frozen (spec only — not yet built) | Configuration | (future: SREMA Verify, other products) | — | Orchestration only, no in-house biometric/OCR; state machine validated against real Stripe/Persona behavior |
| Evidence, Notifications, AI, SMS, Analytics, Logs, API Gateway, Admin Dashboard | — | ⚪ Not started | — | — | — | Boundary-level only, see §9 |
...
## 20. Verify Module Specification v1.0 — STATUS: FROZEN (stress-tested across two passes, including live research against real Stripe Identity and Persona status models before freezing the state machine; ready for agent validation and build)
...
- Six modules frozen and built: Auth v1.1, Organizations v1.0, Configuration v1.0, Mail v1.0, Storage v1.0, Pay v1.0 (306/306 tests).
- Verify v1.0 (§20): DRAFT — ownership and data-minimization principle agreed; full interface incorporates that agreement plus the state machine, webhook handling, and idempotency reasoning already established for Pay. Pending its own stress-test pass before freeze.

**Next steps, in order:**
1. Debate this Verify draft (§20) — same review-board process used for every prior module. Particular attention to: whether the normalized status set (pending/in_review/approved/rejected/expired) is sufficient for real provider webhook event types, whether `verificationType` should be a constrained enum rather than an opaque string, and whether any additional error codes are needed.
2. Freeze the Verify spec.
3. Send the agent to validate — like Storage and Pay, Verify has no existing provisional stub to reconcile against. Step 1 should confirm no frozen module already assumes a different Verify shape, and that Configuration's module catalog can accept provider key entries for whichever provider(s) are targeted first (e.g. Stripe Identity or Persona).
4. Build Verify per §20.
5. Boundary + regression + compliance tests (Rule 12) before Verify can be marked complete — regression must include all 306 existing tests. Given the Data Minimization Rule, expect the same level of scrutiny Pay's PCI compliance tests got — explicit tests proving no document/biometric data path exists anywhere in the code.
6. Only after Verify is reviewed and approved does the Build Order (§13) continue to Evidence, Notifications, AI, etc.
7. Do not pre-specify Evidence, Notifications, etc. beyond §9's boundary-level description until their turn.
