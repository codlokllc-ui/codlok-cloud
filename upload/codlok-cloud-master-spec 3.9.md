...
**Spec Version: 3.9** — Last updated: Dashboard v1.0 IA added as §23 — Products-first navigation, binding no-business-entities display rule (enforces §3.10/§3.12 in the UI itself), Team naming, Secret Templates (copy-not-inherit, preserves §3.7/§16 workspace isolation). Track A (frontend, mock data) approved to build. Track B (Secret Templates backend) explicitly blocked on an undesigned additive Configuration extension — not started.
...
| Dashboard (Track A) | v1.0 | 🟡 IA approved, not built | 9 modules (mock data for now) | — | — | Frontend only; Secret Templates UI mocked pending Track B |
...
## 23. Codlok Cloud Dashboard v1.0 — Architecture (Track A frontend approved; Track B backend item pending)
...
### Build Tracks
- **Track A (approved, build now):** Login, Products page, Product dashboard (Overview), Modules page, module detail pages, Health, Team, AI Builder, Freeze Log, Secret Templates **UI only** (mocked "Apply Template" interaction, no real Configuration call). Mock data throughout — no backend dependency.
- **Track B (blocked, do not start):** Configuration v1.3 draft for platform-owned Secret Templates, per the Backend status note above. Only after this is frozen does Secret Templates' UI get wired to a real backend.

### Coming Soon (explicitly not designed — UI must not imply these work)
OpenAPI, SDK generation, API Explorer — placeholder nav items only. No ownership pass has been done on any of them (consistent with §18's earlier decision not to design AI/Connect/MCP speculatively — same discipline applied to dashboard features).
