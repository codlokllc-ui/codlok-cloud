...
**Spec Version: 1.2** — Last updated: Auth v1.1 (`getUser()` added), §3.8/§3.9 Ownership Rules added, §12 dependency line corrected.
...
## 10. Auth Module Specification v1.1 — STATUS: FROZEN (v1.0 implemented/tested/approved; reopened once for one additive, non-breaking change, re-frozen as v1.1)
...
## 12. Organizations Module Specification v1.0 — STATUS: IMPLEMENTATION VALIDATION

**Status: not frozen.** Architecture, responsibilities, and security rules (Last Owner, Ownership Transfer, Privilege Escalation) were already debated and are not being reopened. What remains is comparing this spec against the real, now-implemented Auth interface before implementation begins:
- Does `Auth.verifySession()` return exactly what Organizations expects?
- Does `Auth.verifyUser()` exist as specified, or does the equivalent Auth function differ?
- Are Auth's error codes sufficient for Organizations' needs, or is one missing?
- Does implementation reveal any ambiguity not caught during the paper review?

If no changes are needed, Organizations moves to Frozen immediately, before implementation. If changes are needed, make the smallest possible revision, then freeze.
