...
**Spec Version: 1.8** — Last updated: Mail spec (§17) stress-tested and frozen — `getDeliveryStatus` now workspace-scoped, cross-provider failover deferred to backlog, idempotency made a binding v1 rule via `idempotencyKey`. Platform Freeze Log added at top of document.
...
| Mail | v1.0 | 🟢 Frozen (spec only — not yet built) | Configuration | Auth, Organizations | — | Cross-provider failover (Resend→SES) not in v1 |
| Storage, Pay, Verify, Evidence, Notifications, AI, SMS, Analytics, Logs, API Gateway, Admin Dashboard | — | ⚪ Not started | — | — | — | Boundary-level only, see §9 |
...
---
## 17. Mail Module Specification v1.0 — STATUS: FROZEN (stress-tested by both AI reviewers; revisions incorporated below; ready for agent validation and build)
...
- Configuration Service v1.0: Frozen (153/153 tests, incl. Auth regression).
- Mail v1.0 (§17): DRAFT — this is the next milestone, not yet debated/frozen.

**Next steps, in order:**
1. Debate this Mail draft (§17) — same review-board process used for Configuration Service. Particular attention to: whether `sendInvitationEmail()`'s proposed signature actually matches the real provisional stub (must be checked, not assumed), and whether the reliability model (queue + retry, hidden from callers) is specified precisely enough to build against.
2. Freeze the Mail spec.
3. Send the agent to validate the frozen spec against the real Auth/Organizations stub usage (Step 1, same pattern as Configuration Service — expect possible conflicts, handle them the same way: smallest-change options, no invented workarounds).
4. Build Mail per §17, replacing the provisional stub without changing Auth's or Organizations' public interfaces.
5. Boundary + regression + compliance tests (Rule 12) before Mail can be marked Frozen — regression must include all 153 existing Auth/Organizations/Configuration tests.
6. Only after Mail is frozen does Phase 2 continue to Storage, Pay, Audit (§13) — each following the same lifecycle: draft spec → debate → freeze spec → agent validates → build → Build Report → review → freeze module → next module.
7. Do not pre-specify Storage, Pay, Verify, etc. beyond §9's boundary-level description until their turn.
