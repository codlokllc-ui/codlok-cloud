...
**Spec Version: 3.7** — Last updated: SMS full interface frozen (§22) after two stress-test passes. Real contradiction fixed (recipient held transiently but never as system-of-record, mirroring Notifications), getDeliveryStatus() removed as redundant with getSms(), delivery state machine corrected (sent is a resting, not guaranteed-final, state — not every provider confirms final delivery), webhook workspace resolution specified via providerMessageId lookup rather than a caller-supplied workspaceId. Ready for agent validation and build.
...
| Mail | v1.2 | 🟢 Frozen | Configuration | Auth, Organizations, Notifications | 48 (409 incl. full platform regression) | Cross-provider failover not in v1; delivery-status transition table undocumented |
| Storage | v1.0 | 🟢 Frozen | Configuration | (future: Verify, Documents, Inspection) | 53 (244 incl. full regression) | No virus scanning, no multipart upload, GC is lazy not callable — all deliberate v1 scope cuts |
| Pay | v1.0 | 🟢 Frozen | Configuration | (future: Verify, Documents, Subscriptions) | 62 (306 incl. full regression) | No multi-currency conversion, no provider failover, no reconciliation tooling — all deliberate v1 scope cuts |
...
| SMS | v1.0 | 🟢 Frozen (spec only — not yet built) | Configuration | Notifications | — | Audit retention is an Open Design Decision; not yet built |
| Evidence, AI, Search, Audit, Jobs/Queue, Analytics, Logs, API Gateway, Admin Dashboard | — | ⚪ Not started | — | — | — | Boundary-level only, see §9 |
...
## 3. Core Specification (the twelve platform rules)
...
---
## 17. Mail Module Specification v1.2 — STATUS: FROZEN (implemented; 48 Mail tests + full 368-test regression passing; sendEmail() additive, all existing functions verified unchanged)
...
## 18. Storage Module Specification v1.0 — STATUS: FROZEN (implemented; 53 Storage tests + 191 existing = 244/244 passing; boundary, immutability, workspace isolation, upload abandonment TTL, logical/physical delete, and compliance tests confirmed per Rule 12)
...
## 19. Pay Module Specification v1.0 — STATUS: FROZEN (implemented; 62 Pay tests + 244 existing = 306/306 passing; boundary, idempotency, webhook deduplication, PCI compliance, state machine, and regression tests confirmed per Rule 12)
...
## 22. SMS Module Specification v1.0 — STATUS: FROZEN (ownership + full interface, two stress-test passes — recipient-persistence contradiction resolved, getDeliveryStatus removed as redundant, state machine corrected against real provider behavior, webhook workspace resolution specified; ready for agent validation and build)
...
- Eight modules frozen and fully built: Auth, Organizations, Configuration, Mail (v1.2), Storage, Pay, Verify, Notifications (409/409 tests).
- SMS (§22): **Full interface frozen** — ownership + two interface stress-test passes complete. Not yet built.

**Next steps, in order:**
1. Send the agent to validate §22 against the real codebase — confirm no frozen module already assumes a different SMS shape, and specifically re-verify Notifications' channel-selection logic actually works against SMS's real interface once it exists (§21 was designed anticipating SMS but never validated against it — check, don't assume).
2. Build SMS per §22.
3. Boundary + regression + compliance tests (Rule 12) before SMS can be marked complete — regression must include all 409 existing tests. Given the opt-out compliance rule, expect the same scrutiny Pay's PCI tests and Verify's data-minimization tests got.
4. Once SMS's Build Report is reviewed and approved, Search, Audit, Jobs/Queue, and AI remain planning-only per §13 — none should be scheduled next by default. Jobs/Queue remains blocked on its unresolved Option A/B fork. Search and Audit have no concrete driving need from any built module yet.
5. Do not pre-specify Search, Audit, Jobs, AI, etc. beyond §9's boundary-level description until a real need drives one of them.
