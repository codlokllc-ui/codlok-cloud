...
**Spec Version: 2.3** — Last updated: Pay (§19) fully specified and frozen across two stress-test passes (ownership, then full interface). New Core Spec rule §3.12 Financial Ownership Rule added. Not yet built.
...
| Pay | v1.0 | 🟢 Frozen (spec only — not yet built) | Configuration | (future: Verify, Documents, Subscriptions) | — | No multi-currency conversion, no capture/cancel functions in v1 |
| Verify, Evidence, Notifications, AI, SMS, Analytics, Logs, API Gateway, Admin Dashboard | — | ⚪ Not started | — | — | — | Boundary-level only, see §9 |
...
## 19. Pay Module Specification v1.0 — STATUS: FROZEN (stress-tested by both AI reviewers across two passes; Pricing Rule and Refund Decision Rule added; Exchange Rate ownership contradiction resolved — record-only, never computed; PCI Boundary Rule kept at module level rather than promoted to Core Spec, since it's a single-module hard boundary, not a recurring cross-module ownership pattern; ready for agent validation and build)
...
- Storage v1.0: Frozen (244/244 tests).
- Pay v1.0 (§19): **FROZEN** — stress-tested across two review passes (ownership, then full interface). Not yet built.

**Next steps, in order:**
1. Send the agent to validate §19 against the real codebase — like Storage, Pay has no existing provisional stub to reconcile against. Step 1 should confirm no frozen module already assumes a different Pay shape, and that Configuration's module catalog has (or can be given) the Stripe key entries Pay will need.
2. Build Pay per §19.
3. Boundary + regression + compliance tests (Rule 12) before Pay can be marked implementation-complete — regression must include all 244 existing tests. Given the PCI Boundary Rule and webhook deduplication, expect extra scrutiny on compliance tests specifically proving raw card data is never touched and duplicate webhooks are true no-ops.
4. Once Pay's Build Report is reviewed and approved, the Build Order (§13) continues to Verify, Evidence, Notifications, AI, etc.
5. Do not pre-specify Verify, Evidence, etc. beyond §9's boundary-level description until their turn.
