...
**Spec Version: 3.4** — Last updated: Mail v1.0 → v1.2, additively adding sendEmail() (arbitrary caller-supplied subject/body, zero business templating). Found via a genuine Blocker Report during Notifications' Step 1 validation — Mail's three template-bound functions couldn't serve Notifications' need for arbitrary business content. Same additive pattern as Auth v1.0→v1.1 (getUser). No existing Mail function changed.
...
| Mail | v1.2 | 🟢 Frozen (v1.2 spec change, not yet re-verified by build) | Configuration | Auth, Organizations, Notifications | 38 (191 incl. regression as of v1.0) | Cross-provider failover not in v1; delivery-status transition table undocumented; sendEmail() added in v1.2, needs implementation + tests |
...
| Notifications | v1.0 | 🟢 Frozen (spec only — not yet built) | Mail, SMS (future), Push (future) | (future: all products) | — | No cross-channel fallback in v1 — deliberate, business-policy decision |
| Evidence, AI, SMS, Analytics, Logs, API Gateway, Admin Dashboard | — | ⚪ Not started | — | — | — | Boundary-level only, see §9 |
...
---
## 17. Mail Module Specification v1.2 — STATUS: FROZEN (v1.0 implemented/tested/approved; v1.1 added getDeliveryStatus fixes; v1.2 additively adds sendEmail() — see below; ready for agent validation and build)
...
## 21. Notifications Module Specification v1.0 — STATUS: FROZEN (ownership agreed; full interface stress-tested twice; all completeness gaps closed; ready for agent validation and build)
...
- Seven modules frozen and built: Auth, Organizations, Configuration, Mail, Storage, Pay, Verify (358/358 tests).
- Notifications (§21): **Draft v2 — full interface written, stress-tested twice (identifying and resolving 3 ownership forks + 3 later completeness gaps).** Ready for freeze, not yet frozen.

**Next steps, in order:**
1. Freeze §21.
2. Send the agent to validate — Notifications has no existing provisional stub to reconcile against. Step 1 should confirm no frozen module already assumes a different Notifications shape, and that Configuration's module catalog can accept whatever provider keys SMS/Push eventually need (Mail's keys already exist from §17).
3. Build Notifications per §21.
4. Boundary + regression + compliance tests (Rule 12) before Notifications can be marked complete — regression must include all 358 existing tests. Given the content-immutability rule, expect the same scrutiny Pay's PCI tests and Verify's data-minimization tests got — explicit tests proving no transformation/truncation/interpolation path exists anywhere in the code.
5. Once Notifications' Build Report is reviewed and approved, the Build Order continues — SMS remains the other strong candidate; Jobs/Queue and AI remain open/unscoped per §13's planning status.
6. Do not pre-specify SMS, Search, Audit, Jobs, AI, etc. beyond §9's boundary-level description until their turn.
